class HermesLiveMonitorCard extends HTMLElement {
  ...
  _authLine(idx, role) {
    const login=this._state(`sensor.hermes_live_openai_codex_auth${idx}_login`); const quota=this._state(`sensor.hermes_live_openai_codex_auth${idx}_quota_state`);
    const c=quota==='429' ? '#ff5f5f' : (login==='ok' ? '#58d26b' : '#ff5f5f');
    return `<div class="authline"><b>Auth${idx}</b><span>${role}</span><span style="color:${c}">Login ${login} · Quota ${quota}</span></div>`;
  }
