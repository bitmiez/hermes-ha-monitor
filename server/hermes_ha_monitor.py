...
def push_sensors(base_url: str, token: str) -> dict[str, Any]:
    ...
    if cpu is not None:
        cpu_attrs = {'friendly_name': 'Hermes Host CPU', 'unit_of_measurement': '%', 'device_class': 'power_factor', 'state_class': 'measurement', 'icon': 'mdi:cpu-64-bit', 'meaning': 'non-idle CPU time including iowait', 'cpu_work_pct_excluding_iowait': cpu_state.get('work_pct'), 'iowait_pct': cpu_state.get('iowait_pct'), 'core_count': cpu_state.get('core_count'), 'core_pcts': cpu_state.get('core_pcts')}
        ha_put(base_url, token, 'sensor.hermes_host_cpu_usage', cpu, cpu_attrs)
        if cpu_state.get('iowait_pct') is not None:
            ha_put(base_url, token, 'sensor.hermes_host_iowait_usage', cpu_state.get('iowait_pct'), {'friendly_name': 'Hermes Host I/O Wait', 'unit_of_measurement': '%', 'device_class': 'power_factor', 'state_class': 'measurement', 'icon': 'mdi:harddisk-clock', 'cpu_work_pct_excluding_iowait': cpu_state.get('work_pct'), 'cpu_pressure_pct_including_iowait': cpu})
        for idx, core_pct in enumerate(cpu_state.get('core_pcts') or []):
            if core_pct is None:
                continue
            ha_put(base_url, token, f'sensor.hermes_host_cpu_core_{idx}_usage', core_pct, {'friendly_name': f'Hermes Host CPU Core {idx + 1}', 'unit_of_measurement': '%', 'device_class': 'power_factor', 'state_class': 'measurement', 'icon': 'mdi:chip', 'core_index': idx, 'meaning': 'per-core non-idle CPU time including iowait', 'cpu_work_pct_excluding_iowait': (cpu_state.get('core_work_pcts') or [None])[idx] if idx < len(cpu_state.get('core_work_pcts') or []) else None})
        pushed.append('sensor.hermes_host_cpu_usage')

    mem, mem_raw = mem_percent()
    if mem is not None:
        ha_put(base_url, token, 'sensor.hermes_host_ram_usage', mem, {'friendly_name': 'Hermes Host RAM', 'unit_of_measurement': '%', 'state_class': 'measurement', 'icon': 'mdi:memory', 'total_bytes': mem_raw.get('MemTotal'), 'available_bytes': mem_raw.get('MemAvailable')})
    ...
    for name, path in [('root', '/'), ('data', '/data')]:
        ...
        if pct is not None:
            ha_put(base_url, token, f'sensor.hermes_host_disk_{name}_usage', pct, {'friendly_name': f'Hermes Host Disk {path}', 'unit_of_measurement': '%', 'state_class': 'measurement', 'icon': 'mdi:harddisk', 'path': path, **raw})
    ...
    ha_put(base_url, token, 'sensor.hermes_openai_codex_fallback_state', 'active' if fallback_active else 'inactive', {'friendly_name': 'OpenAI Codex Fallback', 'icon': 'mdi:backup-restore', 'primary_auth': primary_auth, 'fallback_auth': fallback_auth, 'primary_429': primary_429, 'fallback_login_ok': fallback_login_ok, 'reason': fallback_reason})
